...
def getSpeciesList(lat, lon, week, threshold=0.05, sort=False):
    ...
    if sort:
        slist = sorted(slist, key=operator.itemgetter(1))

    return slist

def saveSpeciesList(results, path):
    out_string = ''

    if cfg.RESULT_TYPE == 'csv':
        # CSV output file
        header = 'Scientific name,Common name,Probability\n'

        # Write header
        out_string += header

        for s in results:
            out_string += '{},{},{}\n'.format(s[1].split('_')[0], s[1].split('_')[1], s[0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rtype', default='list', help='Specifies output format. Values in [\'list\', \'csv\']. Defaults to \'list\' (Species list).')
    ...
    if not cfg.RESULT_TYPE in ['list', 'csv']:
        cfg.RESULT_TYPE = 'list'
    ...
    # Save species list
    saveSpeciesList(species_list, cfg.OUTPUT_PATH)
